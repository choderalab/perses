...
solvent_delivery = PointMutationExecutor(resource_filename('perses', os.path.join('data', 'kinase-mutation',
                                                                                  'NTRK1.pdb')),
                                         '1',
                                         '613',
                                         'VAL',
...
htfs = {'apo': apo_htf, 'complex': complex_htf}  # Storing htfs in dictionary for convenience
...
# Run simulation for phases apo and complex
for phase in ('apo', 'complex'):
    htf = htfs[phase]  # get phase htf
    ...
    for step in range(nsteps_eq):
        ...
        if step % save_freq_eq == 0:
            _logger.info(f'Phase: {phase}, Step: {step}, equilibrating at lambda = 0, took: {elapsed_time / unit.seconds} seconds')
    ...
    for fwd_step in range(nsteps_neq):
        ...
        if fwd_step % save_freq_neq == 0:
            _logger.info(f'Phase: {phase}, forward NEQ step: {fwd_step}, took: {elapsed_time / unit.seconds} seconds')
            pos = context.getState(getPositions=True, enforcePeriodicBox=False).getPositions(asNumpy=True)
            old_pos = np.asarray(htf.old_positions(pos))
            new_pos = np.asarray(htf.new_positions(pos))
    ...
    for step in range(nsteps_eq):
        ...
        elapsed_time = (time.time() - initial_time) * unit.seconds
        if step % save_freq_eq == 0:
            _logger.info(f'Phase: {phase}, Step: {step}, equilibrating at lambda = 1, took: {elapsed_time / unit.seconds} seconds')
            pos = context.getState(getPositions=True, enforcePeriodicBox=False).getPositions(asNumpy=True)
            old_pos = np.asarray(htf.old_positions(pos))
            new_pos = np.asarray(htf.new_positions(pos))
    ...
    for rev_step in range(nsteps_neq):
        ...
        if rev_step % save_freq_neq == 0:
            _logger.info(f'Phase: {phase}, reverse NEQ step: {rev_step}, took: {elapsed_time / unit.seconds} seconds')
            pos = context.getState(getPositions=True, enforcePeriodicBox=False).getPositions(asNumpy=True)
            old_pos = np.asarray(htf.old_positions(pos))
            new_pos = np.asarray(htf.new_positions(pos))
    ...
    # Save works
    with open(os.path.join(out_path, f"{phase}_forward.npy"), 'wb') as out_file:
        np.save(out_file, forward_works_master)
    with open(os.path.join(out_path, f"{phase}_reverse.npy"), 'wb') as out_file:
        np.save(out_file, reverse_works_master)

    # Save trajs
    with open(os.path.join(out_path, f"{phase}_forward_eq_old.npy"), 'wb') as out_file:
        np.save(out_file, np.array(forward_eq_old))
    with open(os.path.join(out_path, f"{phase}_forward_eq_new.npy"), 'wb') as out_file:
        np.save(out_file, np.array(forward_eq_new))
    with open(os.path.join(out_path, f"{phase}_reverse_eq_new.npy"), 'wb') as out_file:
        np.save(out_file, np.array(reverse_eq_new))
    with open(os.path.join(out_path, f"{phase}_reverse_eq_old.npy"), 'wb') as out_file:
        np.save(out_file, np.array(reverse_eq_old))
    with open(os.path.join(out_path, f"{phase}_forward_neq_old.npy"), 'wb') as out_file:
        np.save(out_file, np.array(forward_neq_old))
    with open(os.path.join(out_path, f"{phase}_forward_neq_new.npy"), 'wb') as out_file:
        np.save(out_file, np.array(forward_neq_new))
    with open(os.path.join(out_path, f"{phase}_reverse_neq_old.npy"), 'wb') as out_file:
        np.save(out_file, np.array(reverse_neq_old))
    with open(os.path.join(out_path, f"{phase}_reverse_neq_new.npy"), 'wb') as out_file:
