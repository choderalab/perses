...
def get_perses_network_results(basepath, trajectory_prefix='out'):
    ...
    * Enable user to specify one or more experimental measurements that can be passed to DiffNet to improve the resulting estimate
    ...
    # Get all paths that could contain edges
    #import glob, os
    #paths = [ os.path.dirname(filename) for filename in glob.glob(f'basepath/*/atom_mapping.png') ]
...
if __name__ == '__main__':
    ...
    # NOTE: If you want to extract the MLE estimate of absolute free energies (up to an additive constant), use this idiom:
    for ligand_name, data in perses_graph.nodes(data=True):
        print(f"{ligand_name:46s} : {data['mle_g_i']:8.2f} +- {data['mle_dg_i']:8.2f} kT")
