#!/bin/env python
"""
Filter CDD export for sulfonamides

https://app.collaborativedrug.com/vaults/5549/collections
collection: sulfonamides
...
cdd_csv_filename = 'CDD CSV Export - 2023-01-20 23h18m20s.csv'
...
df['SMILES'] = df['SMILES'].astype(str).apply(strip_salts)
...
for molecule_name, molecule_group in df.groupby('Canonical PostEra ID'):
    print(molecule_name)
...
df = df.groupby('Canonical PostEra ID').first()
print(df)
print(f'{len(df)} records remain')
