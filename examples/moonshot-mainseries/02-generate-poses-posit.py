...
def generate_restricted_conformers(receptor, refmol, mol, core_smarts=None):
    ...
    receptor : openeye.oechem.OEDesignUnit
        OEDesignUnit (already prepped for docking) for identifying optimal pose
    ...
    logging.debug(f'mol: {oechem.OEMolToSmiles(mol)}')
    ...
    posit_result = oedocking.OEPositResults()
    num_poses = 1
    ret_code = poser.Dock(posit_result, mol, num_poses)

    posed_mols = []
    posit_probs = []
    posit_methods = []
    for single_result in posit_result.GetSinglePoseResults():
        posed_mols.append(single_result.GetPose())
        posit_probs.append(single_result.GetProbability())
        posit_methods.append(
            oedocking.OEPositMethodGetName(single_result.GetPositMethod())
        )
    print(posed_mols)
    print(posit_probs)
    print(posit_methods)
    ...
    class Pose(object):
        def __init__(self, conformer):
            ...
            self.posit_score = None
            self.posit_method = None
            self.docking_score = None
            self.overlap_score = None

    poses = list()
    for single_result in posit_result.GetSinglePoseResults():    
        pose = Pose(single_result.GetPose())
        pose.posit_score = single_result.GetProbability()
        pose.posit_method = single_result.GetPositMethod()
        poses.append(pose)

    # Score clashes
    protein = oechem.OEGraphMol()
    receptor.GetProtein(protein)
    bump_check = BumpCheck(protein)
    for pose in poses:
    ...
    # Set original molecule name
    oechem.OESetSDData(mol, 'parent_molecule', mol.GetTitle().rsplit('_')[0])
...
def generate_restricted_conformers_star(args):
    ...
    mol = generate_restricted_conformers(*args)
...
def generate_restricted_conformers_best(args):
    ...
    # Dock molecules
    mols = [ generate_restricted_conformers(receptor, refmol, mol) for mol in mols ]
...
def read_design_unit(fragid, assembly_state, protonation_state):
    """
    Read the design unit for the given Fragalysis structure
    ...
    receptor_filename = f'./receptors/dimer/Mpro-{fragid}_bound-{protonation_state}.oedu'
        ...
        for fragment in fragments:
            ...
            for assembly_state in ['dimer']:
                ...
                for receptor_protonation_state in receptor_protonation_states:
                    ...
                    du = read_design_unit(fragment, assembly_state, receptor_protonation_state)                    
                    ...
                    generate_poses(du, refmol, molecules_to_dock, output_filename, expand_protonation_states=expand_protonation_states)
