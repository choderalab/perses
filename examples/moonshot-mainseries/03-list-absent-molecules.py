...
def expand_protonation_states(mols):
    ...
    expanded_mols = load_molecules('output.sdf')
...
if __name__ == '__main__':
    ...
    for prefix in molecule_sets_to_dock:
        ...
        target_molecules_filename = 'molecules/' + prefix + f'.csv'
        ...
        # Read CSV
        import pandas as pd
        df = pd.read_csv(target_molecules_filename)

        # Read docked molecules
        ndocked = 0
        nundocked = 0
        docked_molecules = load_molecules('docked/sulfonamides-P1062_0A-dimer-His41(0)-Cys145(0)-His163(0).sdf')
        docked_molecule_titles = set()
        for docked_molecule in docked_molecules:
            title = docked_molecule.GetTitle().rsplit('_')[0]
            docked_molecule_titles.add(title)

        for target_molecule in target_molecules:
            title = target_molecule.GetTitle()
            smiles = df.loc[df.Title==title, "SMILES"].values[0]
            if title not in docked_molecule_titles:
                print(f'{title:20s} {smiles}')
                nundocked += 1
            else:
                ndocked += 1
                

        print(f'{ndocked} docked, {nundocked} not docked')
