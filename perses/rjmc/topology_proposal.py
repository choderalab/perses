...
class PolymerProposalEngine(ProposalEngine):
    ...
    def propose(self,
        ...
        extra_sidechain_map : dict, key: int, value: int, default None
            ...
            note that this will not be checked to ensure chirality is preserved, so use this argument with caution and
            make sure to check the rendered png to be sure that the atom mapping is reasonable.
    ...
    def _construct_atom_map(self,
        ...
        2) Map CBs, HBs, and CGs.
        3) Also maps HG2s, HG3s, and CDs, but not CD1, CD2, HG, HG1, HG11, HG12, HG13, HG21, HG22, HG23 (as mapping the latter atoms may match dissimilar geometries)
        by checking for atom name matches.
        ...
        new_topology : simtk.openmm.app.Topology
            topology of new system
        extra_sidechain_map : dict, key: int, value: int, default None
            map of new to old sidechain atom indices to add to the local_atom_map generated by AminoAcidAtomMapper.
            note that this will not be checked to ensure chirality is preserved, so use this argument with caution and
            make sure to check the rendered png to be sure that the atom mapping is reasonable.
        ...
        from perses.rjmc.atom_mapping import AminoAcidAtomMapper, AminoAcidAtomMapping
        ...
        # Set names of the oemols
        old_oemol.SetTitle(old_res_name)
        new_oemol.SetTitle(new_res_name)
        ...
        # Retrieve mapping of oemol to openmm topology indices for old and new residues
        # old_oemol_to_res_map : dict, key : int old atom index (oemol), value : int old atom index (openmm topology)
        # new_oemol_to_res_map : dict, key : int new atom index (oemol), value : int new atom index (openmm topology)
        old_oemol_to_res_map = {old_oemol.GetAtom(oechem.OEHasAtomName(atom.name)).GetIdx(): atom.index for atom in old_res.atoms()}
        new_oemol_to_res_map = {new_oemol.GetAtom(oechem.OEHasAtomName(atom.name)).GetIdx(): atom.index for atom in new_res.atoms()}

        # Convert oemol indices to openmm topology indices
        local_atom_map = {new_oemol_to_res_map[new_oemol_idx]: old_oemol_to_res_map[old_oemol_idx] for new_oemol_idx, old_oemol_idx in atom_mapping.new_to_old_atom_map.items()}
        ...
        old_res_to_oemol_map = {openmm_idx: oemol_idx for oemol_idx, openmm_idx in old_oemol_to_res_map.items()}
        new_res_to_oemol_map = {openmm_idx: oemol_idx for oemol_idx, openmm_idx in new_oemol_to_res_map.items()}

        # Validate extra_sidechain_map and update local_atom_map
