...
class SolventFEPSetup(object):
    """
    Helper class for setting up the solvent phase for relative FEP calculations.
    """
    ...

class ComplexFEPSetup(object):
    ...
...
class RelativeFEPSetup(object):
    ...
    def __init__(self,
        ...
        transformation_atom_map : dict, optional, default=None
            Atom map for the transformation. If not specified the map will be generated by the `AtomMapper` class.
            Requires atom_map[new_index] = old_index.
        """
        ...
        # Dictionary to store topology proposals and positions -- accessible by phase as key
        self.topology_proposals = {}
        self.old_positions = {}
        self.new_positions = {}
        ...
        # load receptor topology and positions if specified -- not required for solvent phases
        if receptor:
            self._receptor_topology, self._receptor_positions = self._process_receptor(receptor, receptor_positions)
        else:
            _logger.info("No receptor specified. Assuming non-complex phase.")
        ...
        self._setup_phases(
            phases,
            atom_map=transformation_atom_map,
    ...
    def _setup_phases(self, phases, atom_map=None, transform_waters_into_ions_for_charge_changes=True,):
        """Generates a single topology proposal and sets attributes for the different specified phases."""
        # if we are running multiple phases, we only want to generate one topology proposal, and use the same one for
        # the other legs this is tracked using _proposal_phase
        if 'complex' in phases:
            self._setup_complex_phase(atom_map=atom_map,
                                      transform_waters_into_ions_for_charge_changes=transform_waters_into_ions_for_charge_changes)
        if 'solvent' in phases:
            self._setup_solvent_phase(atom_map=atom_map,
                                      transform_waters_into_ions_for_charge_changes=transform_waters_into_ions_for_charge_changes)
        if 'vacuum' in phases:
            self._setup_vacuum_phase(atom_map=atom_map)

    def _setup_complex_phase(self, atom_map=None, transform_waters_into_ions_for_charge_changes=True):
        """
        Setups complex phase. Runs setup on the protein/receptor file for relative simulations
        """
        assert self._receptor_topology is not None, "Setting up complex phase requires a receptor to be specified."
        beta = 1.0 / (kB * self._temperature)
        receptor_md_topology_old = md.Topology.from_openmm(self._receptor_topology)

        complex_md_topology_old = receptor_md_topology_old.join(self._ligand_md_topology_old)
        ...
        if self._spectator_filenames:
            for i, spectator_topology in enumerate(self._spectator_md_topologies,1):
                ...
                self._complex_md_topology_old = complex_md_topology_old.join(spectator_topology)
                n_atoms_spectators += spectator_topology.n_atoms
        complex_topology_old = complex_md_topology_old.to_openmm()

        n_atoms_total_old = complex_topology_old.getNumAtoms()
        ...
        # FIXME: Couldn't this be merged into the previous conditional?
        if self._use_given_geometries:
            ...
            if not self._complex_topology_proposal.unique_new_atoms:
                assert self._geometry_engine.forward_final_context_reduced_potential == None, \
                    f"There are no unique new atoms but the geometry_engine's final context reduced potential is not" \
                    f" None (i.e. {self._geometry_engine.forward_final_context_reduced_potential})"
                assert self._geometry_engine.forward_atoms_with_positions_reduced_potential == None, \
                    f"There are no unique new atoms but the geometry_engine's forward " \
                    f"atoms-with-positions-reduced-potential in not None (i.e. " \
                    f"{self._geometry_engine.forward_atoms_with_positions_reduced_potential})"
                self._complex_added_valence_energy = 0.0
            else:
                self._complex_added_valence_energy = \
                    self._geometry_engine.forward_final_context_reduced_potential - \
                    self._geometry_engine.forward_atoms_with_positions_reduced_potential

            if not self._complex_topology_proposal.unique_old_atoms:
                assert self._geometry_engine.reverse_final_context_reduced_potential == None, \
                    f"There are no unique old atoms but the geometry_engine's final context reduced potential is not " \
                    f"None (i.e. {self._geometry_engine.reverse_final_context_reduced_potential})"
                assert self._geometry_engine.reverse_atoms_with_positions_reduced_potential == None, \
                    f"There are no unique old atoms but the geometry_engine's atoms-with-positions-reduced-potential " \
                    f"in not None (i.e. {self._geometry_engine.reverse_atoms_with_positions_reduced_potential})"
                self._complex_subtracted_valence_energy = 0.0
            else:
                self._complex_subtracted_valence_energy = \
                    self._geometry_engine.reverse_final_context_reduced_potential - \
                    self._geometry_engine.reverse_atoms_with_positions_reduced_potential
        ...
        # Storing objects in public and convenient dictionaries
        self.topology_proposals["complex"] = self.complex_topology_proposal
        self.old_positions["complex"] = self.complex_old_positions
        self.new_positions["complex"] = self.complex_new_positions


    def _setup_solvent_phase(self, atom_map=None, transform_waters_into_ions_for_charge_changes=True):
        """
        Setup for the solvent phase. It will try using topology proposal from complex leg if it exists.
        """
        beta = 1.0 / (kB * self._temperature)
        ...
        if self._use_given_geometries:
            ...
            if not self._solvent_topology_proposal.unique_new_atoms:
                ...
                self._solvent_added_valence_energy = \
                    self._geometry_engine.forward_final_context_reduced_potential - \
                    self._geometry_engine.forward_atoms_with_positions_reduced_potential
            ...
            if not self._solvent_topology_proposal.unique_old_atoms:
                ...
                self._solvent_subtracted_valence_energy = \
                    self._geometry_engine.reverse_final_context_reduced_potential - \
                    self._geometry_engine.reverse_atoms_with_positions_reduced_potential
        ...
        # Storing objects in public and convenient dictionaries
        self.topology_proposals["solvent"] = self.solvent_topology_proposal
        self.old_positions["solvent"] = self.solvent_old_positions
        self.new_positions["solvent"] = self.solvent_new_positions

    def _setup_vacuum_phase(self, atom_map=None):
        """
        Setup for the vacuum phase. It will try using topology proposal from other phases if they exist.
        """
        beta = 1.0 / (kB * self._temperature)
        _logger.info(f"Detected solvent...")
        ...
        if self._use_given_geometries:
            ...
            if not self._vacuum_topology_proposal.unique_new_atoms:
                ...
                self._vacuum_added_valence_energy = self._geometry_engine.forward_final_context_reduced_potential - \
                                                    self._geometry_engine.forward_atoms_with_positions_reduced_potential
            ...
            if not self._vacuum_topology_proposal.unique_old_atoms:
                ...
                self._vacuum_subtracted_valence_energy = \
                    self._geometry_engine.reverse_final_context_reduced_potential - \
                    self._geometry_engine.reverse_atoms_with_positions_reduced_potential
        ...
        # Storing objects in public and convenient dictionaries
        self.topology_proposals["vacuum"] = self.vacuum_topology_proposal
        self.old_positions["vacuum"] = self.vacuum_old_positions
        self.new_positions["vacuum"] = self.vacuum_new_positions
