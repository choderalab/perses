...
class HybridCompatibilityMixin(object):
    ...
    def setup(self, n_states, temperature, storage_file,
              minimisation_steps=100, lambda_schedule=None,
              real_endstates=False, lambda_protocol=LambdaProtocol()):
        ...
        if self._real_endstates is True:
            ...
            # taking the first and last states of the alchemical protocol
            unsampled_endstates = [thermodynamic_state_list[0],thermodynamic_state_list[-1]]
...
class HybridSAMSSampler(HybridCompatibilityMixin, sams.SAMSSampler):
    """
    SAMS that supports unsampled end states with a different number of atoms
    """
...
class HybridRepexSampler(HybridCompatibilityMixin, replicaexchange.ReplicaExchangeSampler):
    """
    ReplicaExchangeSampler that supports unsampled end states with a different number of atoms
    """
