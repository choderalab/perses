...
class AnalyticalBeadSystems(object):
    ...
    def run_rj_simple_system(self, configurations_initial, topology_proposal, n_replicates):
        ...
        """
        import tqdm
        from perses.rjmc.experimental_geometry import FFAllAngleGeometryEngine
        final_positions = []
        atom_placement_list = []
        ...
        for _replicate_idx in tqdm.trange(n_replicates):
            ...
            _new_positions, _lp, atom_placements, final_context_reduced_potential = _geometry_engine.propose(topology_proposal, _old_positions, beta)
            atom_placement_list.append(atom_placements)
            ...
            logPs[_replicate_idx, 2] = 0.0
            final_rp = self.compute_rp(topology_proposal.new_system, _new_positions)
            # print("initial rp: {}".format(_initial_rp))
            # print("final_rp: {}".format(final_rp))
            logPs[_replicate_idx, 3] = final_rp
            final_positions.append(_new_positions)
        return logPs, final_positions, atom_placement_list
    ...
    def forward_transformation(self, iid_positions_A, printer=False):
        ...
        _data_forward, _proposed_positions, atom_placements = self.run_rj_simple_system(iid_positions_A, _topology_proposal, self.num_iterations)
        ...
        return proposed_positions, atom_placements
...
# %%
def test_AnalyticalBeadSystems(transformation=[[4,5]], num_iterations=1000):
    """
    ...
    for pair in transformation:
        ...
        _proposed_positions, atom_placements = test.forward_transformation(_iid_positions_A, printer=False)

        return np.array(atom_placements), test.work_forward, _iid_positions_A_stacked, test.sys_pos_top['A'][-1]

atom_placements, work_forward, initial_positions, topology = test_AnalyticalBeadSystems()

#atom_placements.shape
work_forward
# %%
...
#this should verify that we are generating works the same way with both methods
logp_phis = atom_placements[:,0,7]
initial_energies = atom_placements[:,0,-1]
ur_utheta_summation = atom_placements[:,0,1] + atom_placements[:,0,2]
du = np.array([i for i in atom_placements[:,0,9]])
logp_rs = atom_placements[:,0,5]
logp_thetas = atom_placements[:,0,6]
dets = atom_placements[:,0,8]
logZs_r_theta = (logp_rs + logp_thetas - dets + ur_utheta_summation)
logZ_phis = logp_phis + (du - ur_utheta_summation)
assert all(abs(logZs_r_theta + logZ_phis - work_forward) < 1e-5)
# %%
#now we can actually explore what is happening if we make relevant changes or such!!!


#first thing to do is grab the angles from the original structure
import mdtraj as md
t = md.Trajectory(initial_positions, topology)
thetas = md.compute_angles(t, np.array([[1,2,3]]))[:,0]
phis = md.compute_dihedrals(t, np.array([[0,1,2,3]]))[:,0]

# %%
#now we can grab the proposed angles...
new_thetas = atom_placements[:,0,3]
new_phis = atom_placements[:,0,4]
#and we can plot
...
sns.scatterplot(phis, logZ_phis).set(xlabel = "theta", ylabel = 'lnZ_phi')
plt.savefig("/home/dominic/Downloads/analytical_branch_phi_lnZ_phi.pdf")
