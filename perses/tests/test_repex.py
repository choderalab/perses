...
def test_RESTCapableHybridTopologyFactory_repex_charge_transformation():
    """
    Check whether the free energies for two different neutral -> negative -> neutral transformations are equal
    to make sure that introducing a counterion is self consistent.
    - Transformation 1: O=Cc1ccccc1 -> O=C([O-])c1ccccc1 -> O=Cc1ccccc1 (benzaldehyde to benzoate to benzaldehyde)
    - Transformation 2: CC(=O)c1ccccc1 -> O=C([O-])c1ccccc1 -> CC(=O)c1ccccc1

    We do this because the free energy of benzaldehyde->benzoate will not be equal and opposite of that of benzoate->benzaldehyde.
    There will be an energy mismatch because we introduce counterions by turning a water into an ion, so the benzoate system
    in the first leg is not the same as in second leg.
    ...
    n_iterations = 2
    n_states = 36
    d_transformations = {'forward': [(2, 4), (3, 4)], 'reverse': [(4, 2), (4, 3)]}  # Ligand indices in perses/data/test.smi

    with enter_temp_directory() as temp_dir:
        for transformation_type, transformations in d_transformations.items():
            for ligand_A_index, ligand_B_index in transformations:
                # Generate topology proposal and positions
                smiles_filename = resource_filename("perses", os.path.join("data", "test.smi"))

                # smiles_filename = resource_filename("perses", os.path.join("data", "test.smi")) if transformation_type == 'forward' else os.path.join(temp_dir, f"{ligand_A_index}.cif")
                fe_setup = RelativeFEPSetup(
                    ligand_input=smiles_filename,
                    old_ligand_index=ligand_A_index,
                    new_ligand_index=ligand_B_index,
                ...
                if transformation_type == 'forward':
                    app.PDBxFile.writeFile(htf._topology_proposal.new_topology,
                                           htf.new_positions(htf.hybrid_positions),
                                           open(os.path.join(temp_dir, f"{ligand_B_index}.cif"), "w"),
                                           keepIds=True)
        ...
        DDG = data['2-4']['free_energy'] + data['4-2']['free_energy'] \
              - (data['3-4']['free_energy'] + data['4-3']['free_energy'])
        dDDG = np.sqrt(data['2-4']['error'] ** 2
                       + data['4-2']['error'] ** 2
                       + data['3-4']['error'] ** 2
                       + data['4-3']['error'] ** 2)
