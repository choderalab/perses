...
_logger.setLevel(logging.DEBUG) #reset to 'INFO'
...
class SmallMoleculeSetProposalEngine(ProposalEngine):
    ...
    def propose(self, current_system, current_topology, current_mol=None, proposed_mol=None, current_metadata=None):
        ...
        if not self._atom_map:
            ...
            mol_atom_map = self._get_mol_atom_map(current_mol, proposed_mol, atom_expr=self.atom_expr,
                                                  bond_expr=self.bond_expr, allow_ring_breaking=self._allow_ring_breaking)
        else:
    ...
    @staticmethod
    def _get_mol_atom_map(current_molecule, proposed_molecule, atom_expr=None, bond_expr=None, allow_ring_breaking=True):
        """
    ...
    @staticmethod
    def _narrow_mol_atom_map_by_chirality(current_mol, proposed_mol, new_to_old_atom_map):
        """
        Given two molecules and a specified new_to_old_atom_map, returns the mapping of atoms between them that satisfy prespecified atom stereochemistry conditions.
        Specifically,
        The size of the returned atom map will always be less than or equal to the size of the atom map that is input.
        No mapping reorganization is conducted.
        Perhaps it would be advantageous to use a Strong mapping scheme in the event of chiral mapping
        ...
        proposed_molecule : openeye.oechem.oemol object
             The proposed new molecule
        new_to_old_atom_map : dict
            the dict of new : old atom indices which have previously been mapped regardless of chirality

        Returns
        -------
        chiral_new_to_old_atom_map : dict
            the dict of new : old atom indices which have been filtered for changes in chiral centers
        """
        chiral_new_to_old_atom_map = {}

        _logger.debug(f"\tperceiving chirality for current and proposed oemols")
        oechem.OEPerceiveChiral(current_mol)
        oechem.OEPerceiveChiral(proposed_mol)
        stereos = []

        old_atoms = [atom for atom in current_mol.GetAtoms()]

        for atom in proposed_mol.GetAtoms():
            if atom.GetIdx() in new_to_old_atom_map.keys():
                new_chiral = atom.IsChiral()
                new_stereo = oechem.OEAtomStereo_Undefined
                if atom.HasStereoSpecified(oechem.OEAtomStereo_Tetrahedral):
                    v_new = [nbr for nbr in atom.GetAtoms()]
                new_stereo = atom.GetStereo(v_new, oechem.OEAtomStereo_Tetrahedral)

                # pull the old atom attributes
                old_atom = old_atoms[new_to_old_atom_map[atom.GetIdx()]]
                old_chiral = old_atom.IsChiral()
                old_stereo = oechem.OEAtomStereo_Undefined
                if old_atom.HasStereoSpecified(oechem.OEAtomStereo_Tetrahedral):
                    v_old = [nbr for nbr in old_atom.GetAtoms()]
                old_stereo = old_atom.GetStereo(v_old, oechem.OEAtomStereo_Tetrahedral)

                # if old_chiral and new_chiral: # both new and old atoms are R (S)
                #     if old_stereo == new_stereo:
                #         #if set
                #     elif old_stereo != new_stereo:
                #         #in this case, we have to use some magic to choose which atoms to swap
                # elif (not old_chiral) and (not new_chiral):
                #     #we can skip this for now, but we may want to change in the future (i.e. relative stereoisomers)
                # elif (not old_chiral) and new_chiral:
                #     #we have to decide where to place atoms
                # elif old_chiral and (not new_chiral):
                #     # we don't need to bother with this (probably)
                #     continue
